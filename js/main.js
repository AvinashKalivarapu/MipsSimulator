...
function config() {
  ...
  new Button(55 + 1 * 10 + 1 * 100, 5, 100, 25, 'kruskal\'s MST', '#3652a4', '#FFFFFF', function() { Experiment.loadState(BFS_RUNNING_STATE); }, 15, '#FFFFFF', DRAWN_STATE, false);
  new Button(55 + 2 * 10 + 2 * 100, 5, 100, 25, 'ResetGraph', '#3652a4', '#FFFFFF', function() { Experiment.resetGraph(); }, 15, '#FFFFFF', DRAWN_STATE, false);
  new Button(55 + 3 * 10 + 3 * 100, 5, 100, 25, 'Clear Screen', '#3652a4', '#FFFFFF', function() { Experiment.resetGraph(); }, 15, '#FFFFFF', DRAWN_STATE, false);
  ...
  if(CURRENTSTATE == DFS_RUNNING_STATE || CURRENTSTATE == BFS_RUNNING_STATE) {
    for(i = 0; i < GraphNode.allInstances.length; i++) {
      if(GraphNode.allInstances[i].inside(MOUSE_X, MOUSE_Y)) {
        ...
        if(CURRENTSTATE == DFS_RUNNING_STATE) Experiment.clickedInBFS(i);
      }
    }
  }
}
